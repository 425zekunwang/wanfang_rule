...
注：A：上方两幅图：$+\ 1.00\mathrm{D}$ 和-1.00D 的离焦，点扩散函数完全相同。中间两幅图：加入$0.2\upmu\mathrm{m}$的慧差后，点扩散函数仍然相同。下方两幅图：加入二阶像散后，点扩散函数形态出现区别，一个变成水平伸展，一个垂直伸展。B：用引入二阶散光后的点扩散函数和E 视标卷积，发现卷积后视网膜接收到的图像不同。  
